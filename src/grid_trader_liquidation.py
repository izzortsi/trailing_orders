...
def send_order_grid(client, symbol, open_positions, tp, qty=1.1, side=SIDE, sl=None, protect=False, is_positioned=False):
    ...
    liquidation_price = open_positions[symbol]["liquidation_price"]        
    entry_1 = float(liquidation_price)*(1+0.015)
    
    print(liquidation_price, entry_1)
    entry_2 = (float(open_positions[symbol]["entry_price"]) + entry_1)/2

    # grid_entries = [band.values[-1] for band in entry_bands]
    grid_entries = np.array([entry_1, entry_2])
    print(grid_entries)

    filters = get_filters()
    symbolFilters = filters[symbol]
    base_price = np.max(grid_entries)
...
if __name__ == "__main__":
    ...
    for symbol in open_positions.keys():
        print(symbol)
        sym_pos_risk = futures_client.get_position_risk(symbol=symbol)[0]
        print(sym_pos_risk)
        open_positions[symbol]["liquidation_price"] = float(sym_pos_risk["liquidationPrice"])
        open_positions[symbol]["risk"] = sym_pos_risk
    ...
    grid_entries, price_precision, qty_precision, min_qty, order_size, step_size = send_order_grid(futures_client, PAIR, open_positions, TAKE_PROFIT, qty=1.1, side=SIDE, sl=None, protect=False, is_positioned=False)
    print(grid_entries)
    ...
    if PAIR in open_positions.keys():
        ...
        entry_price = np.max(grid_entries) if side == "SELL" else np.min(grid_entries) 
